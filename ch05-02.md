...
```java
MyFunctionalInterface my3 = (name) -> System.out.println("你好，"+name);
```
...
Consume是一个**消费型接口**。与Supplier接口相反，它不是用于生产一个数据，而是用于消费一个数据。当泛型指定什么类型，就可以使用accept方法消费什么类型的数据。至于具体怎么消费，根据需求实现。
